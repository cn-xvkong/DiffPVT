...
from Res2Net.basic_res2net_decoder import Decoder
from unet.basic_unet_denose import get_timestep_embedding, nonlinearity
...
class DRes2Net(nn.Module):
    def __init__(self, block=Bottle2neck, layers=[3, 4, 6, 3], baseWidth=26, scale=4, num_classes=1000):
        ...
        self.temb_proj256 = torch.nn.Linear(512, 256)
        self.temb_proj512 = torch.nn.Linear(512, 512)
        self.temb_proj1024 = torch.nn.Linear(512, 1024)

        self.Decoder1 = Decoder(1024, 1024, 512)
        self.Decoder2 = Decoder(512, 512, 256)
        self.Decoder3 = Decoder(256, 256, 64)
    ...
    def forward(self, x: torch.Tensor, t, embeddings=None, image=None):
        ...
        x = self.conv1(x)
        x0 = x + self.temb_proj64(nonlinearity(temb))[:, :, None, None]
        x0 += embeddings[0]
        x0 = self.bn1(x0)
        x0 = self.relu(x0)
        x0 = self.maxpool(x0)
        ...
        x1 = self.layer1(x0)
        x1 = x1 + self.temb_proj256(nonlinearity(temb))[:, :, None, None]
        x1 += embeddings[1]

        x2 = self.layer2(x1)
        x2 = x2 + self.temb_proj512(nonlinearity(temb))[:, :, None, None]
        x2 += embeddings[2]

        x3 = self.layer3(x2)
        x3 = x3 + self.temb_proj1024(nonlinearity(temb))[:, :, None, None]
        x3 += embeddings[3]

        x3 = self.maxpool(x3)
        x4 = self.layer4(x3)
        x4 = x4 + self.temb_proj1024(nonlinearity(temb))[:, :, None, None]
        x4 += embeddings[4]

        x43 = self.Decoder1(x4, x3)
        x32 = x43 + self.temb_proj512(nonlinearity(temb))[:, :, None, None]

        x32 = self.Decoder2(x32, x2)
        x21 = x32 + self.temb_proj256(nonlinearity(temb))[:, :, None, None]

        x21 = self.Decoder3(x21, x1)
        output = x21 + self.temb_proj64(nonlinearity(temb))[:, :, None, None]
...
def res2net50_26w_4s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(
            'pretrained/res2net50_26w_4s-06e79181.pth')  # (model_zoo.load_url(model_urls['res2net50_26w_4s']))
    return model
